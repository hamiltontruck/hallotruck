...
begin
  ...
  if current_role is null then raise exception using errcode = 'P0002', message = 'Partner account was not found.'; end if;
  if current_role::text in ('admin', 'ceo') then
    raise exception using errcode = '42501', message = 'Admin and CEO roles are protected and cannot be replaced.';
  end if;
  if current_role::text not in ('customer', 'driver', 'partner') then
    raise exception using errcode = '22023', message = 'Only Customer, Driver or existing Partner accounts can be onboarded.';
  end if;
  if current_role::text <> 'partner' and not p_confirm_role_replacement then
  ...
  if current_role::text <> 'partner' then
  ...
  insert into public.partner_memberships (
    ...
    case when current_role::text = 'partner' then null else current_role end
  ) returning id into membership_id;

  if current_role::text <> 'partner' then
    insert into public.partner_activity_log (partner_id, actor_id, action, entity_type, entity_id, metadata)
    values (p_partner_id, auth.uid(), 'profile_role_changed', 'profile', p_user_id::text,
      jsonb_build_object('previous_role', current_role::text, 'new_role', 'partner'));
  end if;
  insert into public.partner_activity_log (partner_id, actor_id, action, entity_type, entity_id, metadata)
