...
-- Keep Driver self-registration owner-scoped while allowing the descriptive
-- truck model shown in the Admin review form to be submitted during onboarding.
...
declare
  ...
  v_vehicle_type text;
  v_model text := nullif(btrim(regexp_replace(coalesce(p_model, ''), '[[:space:]]+', ' ', 'g')), '');
begin
  ...
  if v_driver_status = 'approved' then
    raise exception 'Approved vehicle details are locked. Contact HALLOTRUCK Admin to change the plate, type, model or capacity.'
      using errcode = '42501';
  end if;
  ...
  if char_length(coalesce(v_model, '')) > 120 then
    raise exception 'Truck model must contain at most 120 characters.' using errcode = '22023';
  end if;
...
-- Preserve every existing web/native caller that still sends the original
-- three arguments, including the model already recorded for the current driver.
create or replace function public.driver_save_vehicle_profile(
  p_plate_number text,
  p_vehicle_type text,
  p_capacity_tons numeric
...
as $function$
  select *
  from public.driver_save_vehicle_profile(
    p_plate_number,
    p_vehicle_type,
    p_capacity_tons,
    coalesce((
...
revoke all on function public.driver_save_vehicle_profile(text, text, numeric, text) from public, anon;
grant execute on function public.driver_save_vehicle_profile(text, text, numeric, text) to authenticated;
