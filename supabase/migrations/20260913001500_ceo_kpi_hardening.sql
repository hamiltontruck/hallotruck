...
begin
  ...
  select jsonb_build_object(
    'partnerCommission', coalesce((
      select sum(greatest(coalesce(e.hallo_commission_etb, 0), 0))
    ...
    'pendingPartnerSettlementAmount', coalesce((
      select sum(greatest(coalesce(s.amount_etb, 0), 0))
      from public.partner_settlements s
      where s.status in ('pending', 'under_review', 'approved', 'partially_paid')
    ), 0),
    'expiringDocuments', (
    ...
    'topPartners', coalesce((
      ...
      from (
        select e.partner_id,
               coalesce(sum(greatest(coalesce(e.gross_etb, 0), 0)), 0) as gross_etb,
               coalesce(sum(greatest(coalesce(e.hallo_commission_etb, 0), 0)), 0) as hallo_commission_etb,
