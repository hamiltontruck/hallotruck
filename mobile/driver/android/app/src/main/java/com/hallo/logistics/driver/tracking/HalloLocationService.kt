...
class HalloLocationService:Service(){
 private val scope=CoroutineScope(SupervisorJob()+Dispatchers.IO);private val remote=TrackingRemoteDataSource();private val fused by lazy{LocationServices.getFusedLocationProviderClient(this)};private var orderId:String?=null
 private val callback=object:LocationCallback(){override fun onLocationResult(result:LocationResult){val id=orderId?:return;result.lastLocation?.let{l->scope.launch{runCatching{remote.record(TrackingPingRequest(id,l.longitude,l.latitude,l.bearing.toDouble(),l.speed*3.6,l.accuracy.toDouble(),Instant.ofEpochMilli(l.time).toString()))}.onFailure{stopSelf()}}}}}
