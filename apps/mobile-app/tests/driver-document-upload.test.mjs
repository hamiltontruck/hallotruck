...
test("validates compact identity and vehicle upload scope", () => {
  assert.doesNotThrow(() => validateVerificationUpload({ documentKey: "license_front", file: { name: "license.pdf", type: "application/pdf", size: 1200 }, truckId: null, expiryDate: "2027-08-31", today: new Date("2026-08-31T00:00:00Z") }));
  assert.doesNotThrow(() => validateVerificationUpload({ documentKey: "vehicle_registration", file: { name: "registration.pdf", type: "application/pdf", size: 1200 }, truckId: "truck-1" }));
  assert.throws(() => validateVerificationUpload({ documentKey: "license_front", file: { name: "license.pdf", type: "application/pdf", size: 1200 }, truckId: "truck-1", expiryDate: "2027-08-31" }), /must not be linked/);
  assert.throws(() => validateVerificationUpload({ documentKey: "insurance", file: { name: "insurance.pdf", type: "application/pdf", size: 1200 }, truckId: "truck-1" }), /compact 8-file/);
});
...
test("enforces MIME, photo-only and 10 MB boundaries", () => {
  ...
  assert.throws(() => validateVerificationUpload({ documentKey: "national_id_front", file: { name: "id.txt", type: "text/plain", size: 1000 }, truckId: null }), /JPG, PNG, WebP or PDF/);
