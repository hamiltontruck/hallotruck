...
test("Driver signup uses the compact eight-file policy and mobile camera formats", () => {
  const onboarding = read("../src/onboarding.tsx");
  ...
  assert.match(onboarding, /8 of 8 required files submitted/);
  assert.match(onboarding, /All eight required files are submitted/);
  assert.match(onboarding, /\['vehicle_registration', 'Vehicle registration'\]/);
  assert.match(onboarding, /\['truck_front', 'Truck photo front'\]/);
  assert.match(onboarding, /\['truck_side', 'Truck photo side'\]/);
  assert.doesNotMatch(onboarding, /\['insurance',/);
  assert.match(onboarding, /\['license_front', 'License front', true\]/);
  assert.match(onboarding, /\['national_id_front', 'National ID front', true\]/);
