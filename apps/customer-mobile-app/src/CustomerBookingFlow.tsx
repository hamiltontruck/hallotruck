...
const BOOKABLE_CARGO_CATEGORIES = CARGO_CATEGORIES.filter((category) => category !== "other");
...
export function CustomerBookingFlow({
  ...
  const cargoCopy = cargoDetailsCopy[language];
  ...
  function selectPackaging(next: PackagingType) {
    setPackagingType(next);
    setSubmitError("");
    if (next === "container_20ft" || next === "container_40ft") {
      ...
      <div className="booking-body customer-booking-body">
        ...
        <div className="customer-booking-grid">
          <label>
            ...
            <select value={cargoCategory} onChange={(event) => { setCargoCategory(event.target.value as CargoCategory); setSubmitError(""); }} required>
              {BOOKABLE_CARGO_CATEGORIES.map((category) => (
                <option key={category} value={category}>{cargoCopy.categories[category]}</option>
              ))}
            </select>
          </label>
          <label>
            <span>{text.packaging} <b>{text.required}</b></span>
            <select value={packagingType} onChange={(event) => selectPackaging(event.target.value as PackagingType)} required>
              {PACKAGING_TYPES.map((packaging) => (
                <option key={packaging} value={packaging}>{cargoCopy.packagingTypes[packaging]}</option>
              ))}
            </select>
          </label>
          <label>
        ...
        >
          <span>
            <strong>{text.additionalDetails}</strong>
            <small>{cargoNotes.trim() ? text.detailsAdded : text.optional}</small>
          </span>
