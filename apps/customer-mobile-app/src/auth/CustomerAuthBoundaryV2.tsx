        ...
        <section className={`customer-entry-card is-${mode}`}>
          ...
          <form className="customer-entry-form" onSubmit={submit} aria-busy={busy}>
            {mode === "signup" && <>
              ...
              <label><span>{text.phone}</span><input type="tel" autoComplete="tel" inputMode="tel" maxLength={13} required disabled={busy} placeholder="09XXXXXXXX / 07XXXXXXXX / +2519XXXXXXXX / +2517XXXXXXXX" value={phone} aria-invalid={touched.phone && !phoneValid} aria-describedby="customer-phone-error" onFocus={bringIntoView} onBlur={() => setTouched((current) => ({ ...current, phone: true }))} onChange={(event) => setPhone(sanitizeEthiopianPhoneInput(event.target.value))} />{touched.phone && !phoneValid && <small id="customer-phone-error" className="customer-entry-field-error">{text.phoneInvalid}</small>}</label>
            </>}
