...
const Metric = ({ label, value }) => h(
  "div",
  { className: "min-w-0 border border-asphalt/10 bg-white p-4" },
  h("p", { className: "font-mono text-[9px] uppercase tracking-wide text-steel" }, label),
  h("p", { className: "mt-3 break-words font-display text-xl font-bold" }, value),
);

function Fixture() {
  const nav = ["Overview", "Projects", "Payments", "Documents", "Activity", "Chat"].map((label) =>
    h("button", { className: "whitespace-nowrap border px-4 py-2 text-xs", key: label }, label),
  );
  return h(
    "main",
    ...
    h(
      "section",
      { className: "px-4 py-5" },
      h("nav", { className: "mb-5 flex max-w-full gap-2 overflow-x-auto pb-2" }, ...nav),
      h(
        "div",
        { className: "grid grid-cols-2 gap-3" },
        h(Metric, { label: "Active projects", value: "4" }),
        h(Metric, { label: "Pending payments", value: "ETB 1,250,000" }),
        h(Metric, { label: "Documents", value: "12" }),
        h(Metric, { label: "Members", value: "8" }),
      ),
      h(
        "section",
        { className: "mt-5 min-w-0 border border-asphalt/10 bg-white" },
        h("h2", { className: "p-4 font-display text-xl font-bold" }, "Project progress"),
        h(
          "div",
          { className: "border-t p-4" },
          h("p", { className: "break-words font-semibold" }, "Cross-border logistics modernization and private document review"),
          h("p", { className: "mt-2 break-all text-xs text-steel" }, "TRANSACTION-" + "9".repeat(90)),
          h("div", { className: "mt-3 h-2 bg-asphalt/10" }, h("div", { className: "h-full bg-amber", style: { width: "75%" } })),
        ),
      ),
    ),
  );
