...
try {
  await db.exec(`
    ...
    create table trucks(id uuid primary key default gen_random_uuid(), driver_id uuid, created_by uuid, plate_number text, vehicle_type text, model text, capacity_tons numeric, status text, updated_at timestamptz default now());
    create table orders(id uuid primary key default gen_random_uuid(), driver_id uuid, truck_id uuid, status public.order_status);
    ...
    insert into trucks(id,driver_id,created_by,plate_number,vehicle_type,capacity_tons,status) values ('${truck}','${driver}','${driver}','AA-10001','Isuzu 5 Ton',5,'inactive');
  `);
  ...
  await db.exec(await sqlFile("20260926174000_driver_onboarding_vehicle_model"));
  await db.query("select set_config('test.actor',$1,false)", [other]);
  await db.query("select driver_save_vehicle_profile($1,$2,$3,$4)", ["OR-20002", "Truck 22 Ton", 22, "  FSR   90  "]);
  const savedVehicle = (await db.query("select plate_number,vehicle_type,model,capacity_tons::text from trucks where driver_id=$1", [other])).rows[0];
  assert.deepEqual(savedVehicle, { plate_number: "OR-20002", vehicle_type: "Truck 22 Ton", model: "FSR 90", capacity_tons: "22" }); passed++;
  await db.query("select driver_save_vehicle_profile($1,$2,$3,$4)", ["OR-20002", "Truck 22 Ton", 22, ""]);
  assert.equal((await db.query("select model from trucks where driver_id=$1", [other])).rows[0].model, null); passed++;
  await db.query("select driver_save_vehicle_profile($1,$2,$3,$4)", ["OR-20002", "Truck 22 Ton", 22, "FSR 90"]);
  await db.query("select driver_save_vehicle_profile($1,$2,$3)", ["OR-20002", "Truck 22 Ton", 25]);
  assert.equal((await db.query("select model from trucks where driver_id=$1", [other])).rows[0].model, "FSR 90"); passed++;
  await db.query("update profiles set driver_status='suspended' where id=$1", [other]);
  await rejects("select driver_save_vehicle_profile($1,$2,$3,$4)", ["OR-20002", "Truck 22 Ton", 25, "NPR"], "42501");
  await db.query("update profiles set driver_status='pending' where id=$1", [other]);
  await db.query("select set_config('test.actor',$1,false)", [admin]);
  await rejects("select driver_save_vehicle_profile($1,$2,$3,$4)", ["AA-90009", "Van", 2, "Hiace"], "P0002");
  await db.query("select set_config('test.actor',$1,false)", [admin]);
