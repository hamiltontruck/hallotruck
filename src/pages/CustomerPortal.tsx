...
export function CustomerPortal() {
  ...
  return (
    <main className="min-h-screen bg-bone text-asphalt">
          ...
          <div className="mt-8 grid gap-4">
            ...
            {data.orders.map((order) => {
              ...
              return <article key={order.id} className="border border-asphalt/10 bg-white p-5 sm:p-6">
                ...
                <div className="mt-5 grid grid-cols-2 gap-4 border-t border-asphalt/10 pt-5 text-sm sm:grid-cols-5"><Info label={c.quote} value={`ETB ${Number(order.price_etb ?? 0).toLocaleString()}`} /><Info label={c.distance} value={order.distance_km ? `${order.distance_km} km` : c.pending} /><Info label={cargoText.load} value={loadValue} /><Info label={c.payment} value={pending ? c.pendingVerification : order.payment_status.replace("_", " ")} /><Info label={c.vehicle} value={order.vehicle_type} /></div>
      ...
      {showOrder && <div className="fixed inset-0 z-50 grid place-items-center bg-asphalt/70 p-4"><form onSubmit={create} className="max-h-[94vh] w-full max-w-xl overflow-y-auto bg-white p-6 sm:p-8"><div className="flex justify-between"><div><p className="font-mono text-[10px] tracking-[.2em] text-emerald-700">{c.smartQuote}</p><h2 className="mt-2 font-display text-2xl font-bold">{c.newTransport}</h2></div><button type="button" onClick={() => setShowOrder(false)} className="text-2xl">×</button></div><div className="mt-6"><CustomerQuoteMap onChange={updateRoute} /></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm">{c.vehicleLabel}<select value={vehicle} onChange={(event) => setVehicle(event.target.value)} className="mt-2 block w-full border border-line bg-white px-4 py-3"><option>Pickup</option><option>Van</option><option>Dry Cargo</option><option>Refrigerated</option><option>Trailer</option></select></label><div className="flex items-end border border-line bg-bone px-4 py-3"><div><p className="text-xs text-steel">{c.estimatedDistance}</p><p className="mt-1 font-mono font-semibold">{distance ? `${distance} km` : c.findRoute}</p></div></div><label className="text-sm">{cargoText.amount}<input value={cargoQuantity} onChange={(event) => setCargoQuantity(event.target.value)} type="number" inputMode="decimal" min="0.1" step="0.1" className="mt-2 block w-full border border-line bg-white px-4 py-3" required /></label><label className="text-sm">{cargoText.unit}<select value={cargoUnit} onChange={(event) => setCargoUnit(event.target.value as CargoUnit)} className="mt-2 block w-full border border-line bg-white px-4 py-3"><option value="ton">{cargoText.ton}</option><option value="quintal">{cargoText.quintal}</option></select></label></div><div className={`mt-4 grid gap-3 border p-4 text-xs sm:grid-cols-2 ${cargoValidation ? "border-route/30 bg-route/5" : "border-emerald-700/20 bg-emerald-50"}`}><div><p className="text-steel">{cargoText.equivalent}</p><p className="mt-1 font-semibold text-asphalt">{cargoTons > 0 ? `${cargoTons.toLocaleString(undefined, { maximumFractionDigits: 3 })} ${cargoText.ton}` : "—"}</p></div><div><p className="text-steel">{cargoText.capacity}</p><p className="mt-1 font-semibold text-asphalt">{selectedCapacity ? `${selectedCapacity} ${cargoText.ton}` : "—"}</p></div>{cargoValidation && <p className="sm:col-span-2 font-semibold text-route">{cargoValidation}</p>}</div><div className="mt-6 bg-asphalt p-5 text-white"><p className="font-mono text-[10px] tracking-widest text-white/45">{c.estimatedQuote}</p><p className="mt-2 font-display text-3xl font-bold text-amber">{quote ? `ETB ${quote.toLocaleString()}` : c.selectRoute}</p><p className="mt-2 text-xs text-white/45">{cargoText.pricing}</p></div><button disabled={busy || !routePoints || Boolean(cargoValidation)} className="mt-5 w-full bg-emerald-700 py-4 font-semibold text-white disabled:opacity-50">{busy ? c.creating : c.confirmCreate}</button></form></div>}
