...
export function CustomerRatingCard({ orderId, driverName }: { orderId: string; driverName: string }) {
  ...
  return (
    <section className="customer-rating-card mt-5 min-w-0 overflow-hidden rounded-2xl border border-amber/30 bg-amber/5 p-4 sm:p-5">
      <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 break-words text-sm text-steel">{driverName} · {t.desc}</p>
        </div>
      ...
      {loading ? <p className="mt-4 text-xs text-steel">…</p> : rating ? <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm"><p className="font-semibold text-emerald-900">{t.saved}: {"★".repeat(rating.score)}</p>{rating.comment && <p className="mt-2 text-steel">{rating.comment}</p>}</div> : <form onSubmit={submit} className="mt-4 min-w-0">
        <div className="grid min-w-0 grid-cols-5 gap-1.5 sm:gap-2" role="radiogroup" aria-label={t.title}>
          {[1, 2, 3, 4, 5].map((value) => (
            <button
              ...
              className={`grid aspect-square min-h-10 min-w-0 w-full place-items-center rounded-xl border text-xl transition sm:text-2xl ${value <= score ? "border-amber bg-amber/15 text-amber-dim" : "border-asphalt/10 bg-white text-steel/45"}`}
            >
              ★
            </button>
          ))}
        ...
        <label className="mt-4 block min-w-0 text-xs font-semibold text-asphalt">
          {t.comment}
          <textarea
            value={comment}
            ...
            className="mt-2 block w-full max-w-full resize-y rounded-xl border border-asphalt/15 bg-white px-4 py-3 text-sm font-normal outline-none focus:border-amber"
          />
        </label>
