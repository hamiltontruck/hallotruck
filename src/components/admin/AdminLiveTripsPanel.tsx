...
export function AdminLiveTripsPanel({ orders, trucks, drivers }: AdminLiveTripsPanelProps) {
  ...
  useEffect(() => {
    if (!activeOrders.length) { setSelectedId(""); return; }
    if (!activeOrders.some((order) => order.id === selectedId)) setSelectedId(activeOrders[0].id);
  ...
  useEffect(() => {
    ...
    async function loadOperationalData() {
      if (!selected) {
        ...
        setTelemetry({ recordedAt: null, speedKmh: null, heading: null, freshness: "OFFLINE" });
        return;
      }
      ...
      setTelemetry({
        ...
        freshness: classifyTrackingFreshness(recordedAt),
      });
    }
  ...
  return (
    <>
          ...
          <div className="overflow-hidden rounded-2xl border border-asphalt/10 bg-white">
            ...
            <div className="divide-y divide-asphalt/10">
              {activeOrders.map((order) => {
                ...
                return <button key={order.id} type="button" onClick={() => setSelectedId(order.id)} className={`w-full p-4 text-left transition sm:px-6 ${order.id === selected?.id ? "bg-amber/10" : "hover:bg-bone"}`}><div className="flex items-center justify-between gap-3"><span className="font-mono text-xs font-semibold">{order.tracking_id}</span><span className="text-[10px] font-semibold capitalize text-emerald-700">{order.status.replace("_", " ")}</span></div><p className="mt-2 text-sm font-medium">{order.pickup_address} → {order.dropoff_address}</p><p className="mt-2 text-[11px] text-steel">{truck?.plate_number ?? order.vehicle_type} · {driver?.full_name ?? driver?.phone ?? "Driver assigned"}</p></button>;
              })}
            </div>
          </div>

          {selected && <div className="min-w-0 overflow-hidden rounded-2xl border border-asphalt/10 bg-white"><div className="p-4 sm:p-6">
            ...
            <section className="my-5 grid grid-cols-2 gap-3 lg:grid-cols-4" aria-label="Live driver telemetry">
              <TelemetryMetric label="GPS status" value={telemetry.freshness} detail={gpsAge(telemetry.recordedAt)} tone={telemetry.freshness} />
              <TelemetryMetric label="Speed" value={telemetry.speedKmh == null ? "—" : `${Math.round(telemetry.speedKmh)} km/h`} detail={telemetry.freshness === "LIVE" ? "Current GPS speed" : "Last known speed"} />
              <TelemetryMetric label="Heading" value={telemetry.heading == null ? "—" : `${Math.round(telemetry.heading)}°`} detail="Driver direction" />
              <TelemetryMetric label="Driver phone" value={selectedDriver?.phone ?? "Unavailable"} detail={selectedTruck?.plate_number ? `Plate ${selectedTruck.plate_number}` : "Plate unavailable"} />
            </section>
      ...
      ) : <div className="rounded-2xl border border-asphalt/10 bg-white p-8 text-center"><div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-emerald-50 text-emerald-700">✓</div><h2 className="mt-4 font-display text-xl font-semibold">No active trips right now</h2><p className="mx-auto mt-2 max-w-lg text-sm text-steel">A trip appears here automatically after Admin assigns an eligible driver and the driver starts sharing GPS.</p></div>}
...
function TelemetryMetric({ label, value, detail, tone }: { label: string; value: string; detail: string; tone?: TrackingFreshness }) {
  return <div className="rounded-xl border border-asphalt/10 bg-bone p-3 sm:p-4"><div className="flex items-center justify-between gap-2"><p className="font-mono text-[9px] uppercase tracking-wider text-steel">{label}</p>{tone && <span className={`rounded-full px-2 py-1 text-[8px] font-bold ${freshnessClass(tone)}`}>{tone}</span>}</div><p className="mt-2 truncate font-display text-base font-bold text-asphalt">{value}</p><p className="mt-1 truncate text-[10px] text-steel">{detail}</p></div>;
}

function FinanceMetric({ label, value, accent }: { label: string; value: number; accent?: "commission" | "net" }) {
  const className = accent === "commission" ? "border-amber/35 bg-amber/10" : accent === "net" ? "border-emerald-200 bg-emerald-50" : "border-asphalt/10 bg-bone";
  return <div className={`min-w-0 rounded-xl border p-3 sm:p-4 ${className}`}><p className="truncate font-mono text-[8px] uppercase tracking-wider text-steel sm:text-[9px]">{label}</p><p className="mt-2 truncate font-display text-sm font-bold text-asphalt sm:text-lg">ETB {value.toLocaleString()}</p></div>;
}
