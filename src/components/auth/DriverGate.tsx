...
export function DriverGate({ children }: { children: ReactNode }) {
  ...
  const isDriverOnboardingPath = normalizedPath === "/driver/profile" || normalizedPath === "/driver/documents";
  ...
  if (role === "driver" && driverStatus !== "approved" && !isDriverOnboardingPath) {
    return <main className="min-h-screen bg-bone grid place-items-center p-5"><section className="bg-white border border-line p-7 max-w-md w-full"><p className="font-mono text-[10px] uppercase tracking-[.18em] text-amber-dim">Smart driver onboarding</p><p className="font-display font-bold text-2xl mt-2">Complete your driver and vehicle profile</p><p className="font-body text-sm text-steel mt-3">Your approved files stay saved. Complete the eight required files: driver photo, license front/back, National ID front/back, vehicle registration, and truck front/side photos. Jobs, Trip and Earnings open after Admin verifies the remaining items.</p><Link to="/driver/profile" replace className="block w-full bg-route text-center text-white py-4 mt-6 font-semibold">Continue onboarding</Link><button onClick={switchAccount} className="w-full border border-asphalt/20 text-asphalt py-3 mt-3 font-semibold">Sign out</button></section></main>;
  }
