...
export function printInvoice(order: AdminOrder, truck?: Truck, driver?: Driver, payments: Payment[] = []) {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${safe(order.tracking_id)} invoice</title><style>body{font:14px Arial;color:#1d222a;padding:24px;max-width:760px;margin:auto}h1{font-size:28px}.brand{color:#d68e25}.row{display:flex;gap:20px;justify-content:space-between;border-bottom:1px solid #ddd;padding:12px 0}.row span{text-align:right}.total{font-size:20px;font-weight:bold}.muted{color:#68707c}button{width:100%;border:0;background:#1d222a;color:white;padding:15px;margin-top:24px;font-weight:bold}@media(max-width:480px){body{padding:18px}.row{display:block}.row span{display:block;text-align:left;margin-top:6px}}@media print{button{display:none}body{padding:0}}</style></head><body><h1>HALLO<span class="brand">TRUCK</span></h1><p class="muted">Smart Logistics invoice / receipt</p><div class="row"><b>Tracking</b><span>${safe(order.tracking_id)}</span></div><div class="row"><b>Customer</b><span>${safe(order.customer_name)} · ${safe(order.customer_phone)}</span></div><div class="row"><b>Route</b><span>${safe(order.pickup_address)} → ${safe(order.dropoff_address)}</span></div><div class="row"><b>Truck / driver</b><span>${safe(truck?.plate_number ?? "Unassigned")} · ${safe(driver?.full_name ?? "Unassigned")}</span></div><div class="row"><b>Status</b><span>${safe(order.status.replace("_"," "))}</span></div><div class="row total"><b>Invoice total</b><span>ETB ${total.toLocaleString()}</span></div><div class="row"><b>Verified paid</b><span>ETB ${paid.toLocaleString()}</span></div><div class="row total"><b>Balance</b><span>ETB ${balance.toLocaleString()}</span></div><p class="muted">Generated ${safe(new Date().toLocaleString())}</p><button type="button" onclick="window.print()">Print / Save as PDF</button></body></html>`;
