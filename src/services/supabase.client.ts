...
function withAdminRealtimeCoalescing(client: ReturnType<typeof createClient>) {
  const rawChannel = client.channel.bind(client);

  client.channel = ((topic: string, params?: Parameters<typeof rawChannel>[1]) => {
...
export const supabase = withAdminRealtimeCoalescing(createClient(supabaseUrl, supabaseAnonKey));
