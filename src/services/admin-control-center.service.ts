...
export interface CeoTopCustomer {
  customer_name: string;
  order_count: number;
}

export interface CeoTopRoute {
  pickup_address: string;
  dropoff_address: string;
  order_count: number;
}

export interface CeoTopPartner {
  partner_id: string;
  gross_etb: number;
  hallo_commission_etb: number;
...
export interface ControlCenterServerSummary {
  ...
  pendingPartnerSettlements: number;
  pendingPartnerSettlementAmount: number;
...
export interface ControlCenterData {
  ...
  topCustomers?: CeoTopCustomer[];
  topRoutes?: CeoTopRoute[];
  topPartners?: CeoTopPartner[];
}
...
export async function getControlCenterData(): Promise<ControlCenterData> {
  ...
  serverSummary.partnerCommission = numberOf(ceo.partnerCommission);
  serverSummary.pendingPartnerSettlements = numberOf(ceo.pendingPartnerSettlements);
  serverSummary.pendingPartnerSettlementAmount = numberOf(ceo.pendingPartnerSettlementAmount);
  serverSummary.expiringDocuments = numberOf(ceo.expiringDocuments);

  const topCustomers = rowsOf(ceo.topCustomers).map((row) => ({
    customer_name: String(row.customer_name ?? "Unknown"),
    order_count: numberOf(row.order_count),
  }));
  const topRoutes = rowsOf(ceo.topRoutes).map((row) => ({
    pickup_address: String(row.pickup_address ?? "Unknown"),
    dropoff_address: String(row.dropoff_address ?? "Unknown"),
    order_count: numberOf(row.order_count),
  }));
  const topPartners = rowsOf(ceo.topPartners).map((row) => ({
    partner_id: String(row.partner_id ?? ""),
    gross_etb: numberOf(row.gross_etb),
    hallo_commission_etb: numberOf(row.hallo_commission_etb),
    freight_count: numberOf(row.freight_count),
  }));

  return {
    // Live CEO previews stay bounded. Exact totals and finance values come from
    // leadership-guarded server reports; no array grows with production tables.
